...
class TransactionBuilder {
    ...
    */
    ...
    // return this.get_potential_signatures().then( (pubkeys)=> {
        ...
        return this.get_required_signatures(my_pubkeys).then( required_pubkeys => {
            ...
            for(let pubkey_string of my_pubkeys) {
                ...
                accountLogin.signTransaction(this, signer_pubkeys_added, required_pubkeys);
                // var private_key = cwallet.getPrivateKey(pubkey_string)
                // if( ! private_key)
    ...
    finalize(){
        ...
        return new Promise((resolve, reject)=> {
            ...
            resolve(Api.database_api().exec("get_dynamic_global_properties", []).then((r) => {
    ...
    */
    ...
    get_potential_signatures(){
        ...
        return Api.database_api().exec( "get_potential_signatures", [tr_object] )
    ...
    sign(chain_id = ChainConfig.chain_id){
        if (!this.tr_buffer) { throw new Error("not finalized"); }
        if (this.signed) { throw new Error("already signed"); }
...
function _broadcast(was_broadcast_callback){
    return new Promise((resolve, reject)=> {
        ...
        if (!this.tr_buffer) { reject(new Error("not finalized")); }
        if (!this.signatures.length) { reject(new Error("not signed")); }
        if (!this.operations.length) { reject(new Error("no operations")); }

        if (!("network_broadcast_api" in Api)) { reject(new Error("Api does not include network_broadcast_api")); }
        var tr_object = ops.signed_transaction.toObject(this);


        return Api.network_broadcast_api()
        .exec( "broadcast_transaction_with_callback", [ {reject, resolve} ,tr_object]).then(function(){
